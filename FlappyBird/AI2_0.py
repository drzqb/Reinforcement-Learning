...
class QLAgent():
    def __init__(self, Graph_Write=False):
        ...
        if FLAGS.mode == 'train':
            ...
            if Graph_Write:
                writer = tf.summary.FileWriter('logs', graph=tf.get_default_graph(), filename_suffix='FB2_0_DQN')
                writer.flush()
            self.sess.run(tf.global_variables_initializer())
            self.cost_hist = []
            self.Reward = []
        elif FLAGS.mode == 'play':
            self.saver.restore(self.sess, 'saved_networks/FB2_0_DQN')
    ...
    def build_net(self):
        ...
        self.q_a = tf.reduce_sum(tf.multiply(self.q_eval, self.a), axis=1)
        self.loss = tf.reduce_mean((tf.squared_difference(self.q_target, self.q_a)))
    ...
    def learn(self):
        ...
        for i in range(BATCH_SIZE):
            if batch_is_terminal[i]:
                q_target[i] = batch_r[i]
            else:
                q_target[i] = batch_r[i] + LAMBDA * np.max(q_next[i])

        _, self.cost = self.sess.run([self.train_op, self.loss],
                                     feed_dict={self.s: batch_s, self.a: batch_a, self.q_target: q_target})
    ...
    def train(self):
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            while not is_terminal:
                ...
                self.agent.store_transition(S, ACTION[A], R, S_next, is_terminal)
            ...
            if step_counter > counter_max and self.agent.epsilon == 1.0:
                ...
                self.agent.saver.save(self.agent.sess, 'saved_networks/FB2_0_DQN')
